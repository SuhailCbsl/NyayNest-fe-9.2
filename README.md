...
1. You MUST be running the DSpace backend (i.e. REST API) locally. The e2e tests will _NOT_ succeed if run against our demo/sandbox REST API (https://demo.dspace.org/server/ or https://sandbox.dspace.org/server/), as those sites may have content added/removed at any time.
...
As you can see above, using `npm` commandline tools means that you should never need to modify the `package.json` manually. _We recommend always using `npm` to keep dependencies updated / in sync._
