...
export class AuthService {
    ...
    return this.authRequestService.postToEndpoint('login', body, options).pipe(
      map((rd: RemoteData<AuthStatus>) => {
        if (hasValue(rd.payload) && rd.payload.authenticated) {
          return rd.payload;
        } else if (rd.statusCode === 429) {
          throw new Error('auth.errors.too-many-attempts');
        } else {
  ...
  public isExternalAuthentication(): Observable<boolean> {
    return this.store.pipe(select(getExternalAuthCookieStatus));
  }
  ...
  public hasValidAuthenticationToken(): Observable<AuthTokenInfo> {
    return this.store.pipe(
      ...
      map((authTokenInfo: AuthTokenInfo) => {
        ...
        const token = isNotEmpty(authTokenInfo)
          ? authTokenInfo
  ...
  getExternalServerRedirectUrl(
    ...
    if (
      ...
      externalServerUrl = location.replace(
        /\?redirectUrl=(.*)/g,
        newRedirectUrl,
      );
    }
  ...
  getShortlivedToken(): Observable<string> {
    return this.isAuthenticated().pipe(
      switchMap((authenticated) =>
        authenticated ? this.authRequestService.getShortlivedToken() : of(null),
      ),
    );
  }
