...
describe('objectCacheReducer', () => {
  ...
  const selfLink1 =
    'https://localhost:8081/api/core/items/1698f1d3-be98-4c51-9fd8-6bfedcbd59b7';
  const selfLink2 =
    'https://localhost:8081/api/core/items/28b04544-1766-4e82-9728-c4e93544ecd3';
  ...
  it('should add the payload to the cache in response to an ADD action', () => {
    ...
    const action = new AddToObjectCacheAction(
      objectToCache,
      timeCompleted,
      msToLive,
      requestUUID,
  ...
  it('should when the ADD_PATCH action dispatched', () => {
    ...
    const newState = objectCacheReducer(testState, action);
    expect(newState[selfLink1].patches.map((p) => p.operations)).toContain(
      patch,
    );
  });
  ...
  it('should add dependent requests on ADD_DEPENDENTS', () => {
    let newState = objectCacheReducer(
      testState,
      new AddDependentsObjectCacheAction(selfLink1, ['new', 'newer', 'newest']),
    );
    ...
    newState = objectCacheReducer(
      newState,
      new AddDependentsObjectCacheAction(selfLink2, ['more']),
    );
  ...
  it('should clear dependent requests on REMOVE_DEPENDENTS', () => {
    let newState = objectCacheReducer(
      testState,
      new RemoveDependentsObjectCacheAction(selfLink1),
    );
    expect(newState[selfLink1].dependentRequestUUIDs).toEqual([]);

    newState = objectCacheReducer(
      newState,
      new RemoveDependentsObjectCacheAction(selfLink2),
    );
    expect(newState[selfLink2].dependentRequestUUIDs).toEqual([]);
  });
});
